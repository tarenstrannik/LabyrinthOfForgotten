...
public class ChangePlaySoundDependingOnVelocity : MonoBehaviour
{
    ...
    private IEnumerator IUpdateAudioParams()
    {
        while(true)
        {
            float volumeScale = (m_doorRb.velocity.magnitude / m_maxVelocity) <= 1 ? (m_doorRb.velocity.magnitude / m_maxVelocity) : 1;
            m_doorAudioSource.volume = m_startVolume * volumeScale;
            m_doorAudioSource.pitch = (m_doorRb.velocity.magnitude / m_maxVelocity) * m_startPitch;
