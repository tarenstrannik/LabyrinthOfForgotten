...
real lambert = max(0, dot(normalWS, direction.xyz));
bakeDiffuseLighting += illuminance * lambert / max(1e-4, direction.w);

real backLambert = max(0, dot(backNormalWS, direction.xyz));
backBakeDiffuseLighting += illuminance * backLambert / max(1e-4, direction.w);
